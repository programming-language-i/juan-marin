...
R/ Concurrencia. ya que un solo núcleo gestiona múltiples tareas alternando el tiempo de ejecución entre ellas.

2. Nombrar dos diferencias entre un proceso y un hilo *(memoria, costo, fallo)*.

R/ Memoria: Los procesos poseen espacios de memoria independientes y aislados, mientras que los hilos dentro de un mismo proceso comparten el mismo espacio de memoria.

Costo de creación y fallo: Los procesos son más pesados y costosos de crear a nivel de sistema operativo, pero un fallo en un proceso no afecta a los demás. Los hilos son ligeros, pero un fallo no controlado en un hilo puede finalizar la ejecución de todo el proceso.

3. ¿En qué estado del ciclo de vida está un hilo que ejecuta `time.sleep(2)`? ¿Qué devuelve `is_alive()`?

R/ El hilo se encuentra en estado bloqueado (TIMED_WAITING). El método is_alive() devuelve True, ya que el hilo continúa activo dentro de su ciclo de vida aunque esté pausado.

4. ¿Qué protege el GIL? ¿Evita las condiciones de carrera en los datos del programa?

R/ El GIL protege la memoria interna e integridad de las estructuras de CPython, garantizando que solo un hilo ejecute bytecode de Python a la vez.No evita las condiciones de carrera sobre las variables o estructuras del programa; esa protección debe implementarse mediante mecanismos de sincronización (como locks). 

5. ¿Por qué 4 hilos que duermen 1 s cada uno tardan ~1 s en total y no ~0,25 s?

R/ Porque time.sleep() libera el GIL durante el periodo de espera. Al ser una operación bloqueante pero fuera del intérprete, los 4 hilos realizan la espera de forma concurrente cada 1S en lugar de secuencial 0.25S.


6. Al crear un hilo por herencia, ¿qué método se sobrescribe y cuál nunca? ¿Por qué?

R/ 
   - Sobrescribe: El método run(), donde se define la lógica que ejecutará el hilo en segundo plano.

   - Nunca se sobrescribe: El método start(), dado que contiene la implementación nativa encargada de solicitar la creación del hilo al sistema operativo antes de invocar internamente a run().


7. ¿Qué pasa con un hilo daemon cuando termina el hilo principal? ¿Qué **no** se ejecuta?   

R/  El hilo daemon se interrumpe de forma abrupta en el momento exacto en que finaliza el hilo principal. Como consecuencia, no se ejecutan las cláusulas "finally" ni las instrucciones de cierre o limpieza de recursos pendientes.

8. Completar la regla del curso: *hilos para , procesos para*.

R/ Hilos para operaciones de entrada y salida
    
   procesos para operaciones intensivas
